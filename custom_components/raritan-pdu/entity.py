...
class RaritanPDUEntity(CoordinatorEntity, Entity):
    ...
    def __init__(self, coordinator: RaritanPDUCoordinator,
        ...
        self._attr_unique_id = f"{self.coordinator.pdu.name}-outlet-{self.outlet_index}-{self.entity_description.key}".replace(" ", "-").lower()
    ...
    #     return f"{self.coordinator.pdu.name}-outlet-{self.outlet_index}-{self.entity_description.key}".replace(" ", "-").lower()
    #
    # @property
